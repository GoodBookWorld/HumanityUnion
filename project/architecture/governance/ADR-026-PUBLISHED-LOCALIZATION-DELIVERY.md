...
20. **Reset 03E.11:** Editorial/API diagnostics do not prove carousel leaf localization. Page `FULLY_LOCALIZED` for locale ≠ en requires every participant-visible Media carousel semantic leaf to be owned (`PLP_ENTITY` / `UI_DICTIONARY` / `BRAND` / `TERMINOLOGY` / `GEOGRAPHY` / `PROTECTED_CANONICAL`) with zero unowned card text and zero translatable `CANONICAL_FALLBACK` carousel leaves — computed from real rendered leaves (`media-carousel-semantic-closure`), not inventory expected counts.  
21. **Reset 03E.13:** Materializer usability for `public_news` is only meaningful for the **same ID set** the live `/media` consumer resolves. Carousel discovery MUST use Web SSR selection (`language=en` + source balance, limit 12) — not newest-N without balance. Batch attach is by canonical identity (`entityType`+`entityId` / `article.id`), never array position. A usable snapshot for entity E must resolve `PUBLISHED_LOCALIZED` for E only.  
22. **Reset 04:** Media is the first adapter on a **universal** PLP publication pipeline (ADR-027). Domains join via adapter registry; core must not embed Media/RSS specifics. Localization remains a publication concern. Remaining domains stay on legacy CT until RESET 05+.
23. **Reset 05:** Initiative/Lifecycle is the second production adapter (`initiative_lifecycle`). Country Initiative/election rail meta (`country-initiative-rail-card__meta`) has explicit CONTROLLED_VOCABULARY + GEOGRAPHY ownership. Blog/Discussion/Participant adapters registered with privacy gates; stage consumers remain CT until live acceptance (`REPLACED_PENDING_ACCEPTANCE`). Full CT removal remains RESET 07.
