...
describe("Pack 02G Task 08D.9 — Official Response + Public Impact + Archive author i18n", () => {
  ...
  it("Civic Archive canonical archive content and IDs stay data-bound", () => {
    ...
    assert.match(editor, /resolveCivicArchiveSectionDisplayLabel\(section\.sectionId/);
    assert.doesNotMatch(editor, /section\.title\s*\|\|/);
    assert.match(editor, /section\.body\.trim\(\)/);
    assert.match(document, /document\.finalArchiveTitle/);
    assert.match(document, /resolveCivicArchiveSectionDisplayLabel\(section\.sectionId/);
    ...
    assert.match(completeness, /resolveCivicArchiveCompletenessSummaryDisplay/);
    assert.doesNotMatch(completeness, /\{completeness\.summary\}/);
    assert.doesNotMatch(editor, /value=\{t\(/);
