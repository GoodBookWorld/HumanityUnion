...
const TEST_PREFIX = createTestId("proposal-workspace");
...
function createTestEmail(suffix: string): string {
  return `${TEST_PREFIX}-${suffix}@proposal-workspace.test`;
}
...
describe("ProposalCreated Workspace projection integration", () => {
  before(async () => {
    resetEventInfrastructureForTests();
  ...
  it("updates Workspace from ProposalCreated through outbox dispatch", async () => {
    clearDomainEventHandlers();

    const user = await registerAndConfirm("Workspace Proposal Member", "flow");
    ...
    assert.equal(projection.recentActivities.length, 3);
    assert.equal(projection.recentActivities[0]?.proposalId, created.proposal.proposalId);
    assert.equal(projection.recentActivities[0]?.referenceType, "proposal");
    assert.equal(projection.recentActivities[1]?.discussionId, discussion.discussion.discussionId);
    assert.equal(projection.recentActivities[2]?.activityId, activity.activity.activityId);

    const overview = await getWorkspaceOverviewForMember(user.memberId);
    assert.equal(overview.recentActivities.length, 3);

    const eventId = buildProposalCreatedEventId(created.proposal.proposalId);
    assert.equal(await isEventProcessed(WORKSPACE_PROPOSAL_CREATED_CONSUMER_ID, eventId), true);
  });

  it("remains idempotent on duplicate ProposalCreated delivery", async () => {
    clearDomainEventHandlers();

    const user = await registerAndConfirm("Duplicate Proposal Member", "duplicate");
    await materializeWorkspaceForMember(user);
    registerWorkspaceHandlers();
  ...
  it("retries consumer after Workspace proposal projection write failure", async () => {
    resetEventInfrastructureForTests();

    const user = await registerAndConfirm("Retry Proposal Member", "retry");
    await materializeWorkspaceForMember(user);
    registerWorkspaceHandlers();
    ...
    setForceWorkspaceProposalUpdateFailureForTests(true);
    await assert.rejects(
      () => handleProposalCreatedWorkspaceProjection(envelope),
      /Forced workspace proposal projection update failure/,
    );
    assert.equal(await isEventProcessed(WORKSPACE_PROPOSAL_CREATED_CONSUMER_ID, eventId), false);

    const beforeRetry = await findWorkspaceProjectionByMemberId(user.memberId);
    assert.ok(beforeRetry);
    assert.equal(
      beforeRetry.recentActivities.some((entry) => entry.proposalId === created.proposal.proposalId),
      false,
    );
    ...
    assert.equal(afterRetry.recentActivities[0]?.proposalId, created.proposal.proposalId);
    assert.equal(await isEventProcessed(WORKSPACE_PROPOSAL_CREATED_CONSUMER_ID, eventId), true);
  });

  it("retries ProposalCreated when Workspace projection is not yet materialized", async () => {
    clearDomainEventHandlers();

    const user = await registerAndConfirm("Ordering Proposal Member", "ordering");
    await deleteWorkspaceProjectionByMemberId(user.memberId);
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (await isEventProcessed(WORKSPACE_PROPOSAL_CREATED_CONSUMER_ID, eventId)) {
        break;
      }
    }
    ...
    assert.ok(projection);
    assert.equal(
      projection.recentActivities.some((entry) => entry.proposalId === created.proposal.proposalId),
      true,
    );
    assert.equal(await isEventProcessed(WORKSPACE_PROPOSAL_CREATED_CONSUMER_ID, eventId), true);
  });

  it("rebuilds Workspace state from MemberRegistered through ProposalCreated replay", async () => {
    clearDomainEventHandlers();

    const user = await registerAndConfirm("Rebuild Proposal Member", "rebuild");
    await materializeWorkspaceForMember(user);
    registerWorkspaceHandlers();
