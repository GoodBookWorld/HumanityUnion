/**
 * Pack 08K.1 / 08K.2.2 — heavy PublicLocalizedPresentation reconciliation
 * + gated residual retry operator (write/provider pipeline).
 *
 * Pack 08K.2.8 — imported ONLY after reconcile-public-localization.ts confirms
 * the mode is not a thin residual diagnostic. Do not import this module from
 * diagnose:localization-residuals.
 *
 * OPERATOR_DEPRECATED_MEMORY_UNSAFE (do not use on Render):
 *   --explain-residuals / --explain-residuals-only
 *   --snapshot-explicit-residual-state
 * Use instead:
 *   pnpm --filter @hu/api diagnose:localization-residuals -- --mongo --residual ...
 *
...
import { parseResidualIdentityArgs } from "../modules/language/public-localization-residual-only-diagnostic.js";
import {
...
export async function runReconcilePublicLocalizationHeavy(): Promise<void> {
  ...
  // Pack 08K.2.8 — explain path remains OPERATOR_DEPRECATED_MEMORY_UNSAFE at import time
  // if somehow reached; refuse rather than run bounded-but-heavy residual explain.
  if (explainResiduals) {
    console.error(
      JSON.stringify({
        ...
        reason:
          "Static import of reconcile-public-localization-heavy pulls language registry service, global-search, content-translation.service, Gemini provider graph, civic stores, and corpus helpers before main() — FULL_CORPUS_HYDRATED=false does not measure import RSS.",
        replacement:
...
export async function disconnectReconcilePublicLocalizationHeavy(): Promise<void> {
