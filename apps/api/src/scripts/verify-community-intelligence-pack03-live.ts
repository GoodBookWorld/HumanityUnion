/**
 * Community Intelligence Pack 03 — isolated live verification.
 *
 * Extends Pack 02 isolation for creation-overlap + Related widget recovery:
 * - `hu_test_*` database only (never `humanity_union_dev`)
 * - Seeds strong-overlap pair + unrelated Initiative
 * - Asserts similarity-check strong/weak contracts
 * - Asserts Assistant creation-context answers (structured reasons + no block)
 * - Optional `--serve` for browser matrix (Web → this API)
 *
 * Usage (from apps/api):
 *   pnpm exec tsx src/scripts/verify-community-intelligence-pack03-live.ts
 *   CI_PACK03_PORT=4000 pnpm exec tsx src/scripts/verify-community-intelligence-pack03-live.ts --serve
 */
...
async function main(): Promise<void> {
  ...
  // Use a form-available activity area so browser creation-overlap Scenario A can
  // reproduce strong overlap without inventing a Mobility option in the UI.
  const baseMeta = {
    category: "Infrastructure and Transport",
    tags: ["cycling", "safety"],
    region: "Verification Region",
    language: "en",
    communitySlug: "ci-pack02-community",
    activityArea: "Infrastructure and Transport",
    countrySlug: "ca",
    regionSlug: "bc",
  ...
  const preferredPort = Number(
    process.env.CI_PACK03_PORT ?? process.env.CI_PACK02_PORT ?? (serveMode ? 4000 : 0),
  );
  ...
  try {
    ...
    const assistantEnvelope = (await assistantResponse.json()) as {
      success: boolean;
      data?: {
        suggestions?: Array<{ suggestedText?: string }>;
    ...
    const creationAssist = await fetch(`${baseUrl}/api/v1/assistant/assist`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
      ...
      body: JSON.stringify({
        ...
        currentDraftExcerpt: `${source.title}\n\n${source.description}`,
      }),
    });
    const creationAssistEnvelope = (await creationAssist.json()) as {
      success: boolean;
      data?: { suggestions?: Array<{ suggestedText?: string }> };
    };
    if (!creationAssist.ok || !creationAssistEnvelope.success) {
      throw new Error(`Creation-context Assistant failed: ${creationAssist.status}`);
    }
    const creationText = JSON.stringify(creationAssistEnvelope).toLowerCase();
    if (creationText.includes("ai thinks they are similar")) {
      throw new Error("Assistant must not explain overlap as AI opinion alone.");
    }
    if (
      !creationText.includes("infrastructure") &&
      !creationText.includes("cycling") &&
      !creationText.includes("transport")
    ) {
      throw new Error("Creation-context Assistant should ground in structured CI reasons.");
    }

    const stopAssist = await fetch(`${baseUrl}/api/v1/assistant/assist`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
    ...
    const stopEnvelope = (await stopAssist.json()) as {
      success: boolean;
      data?: { suggestions?: Array<{ suggestedText?: string }> };
    };
    const stopText = JSON.stringify(stopEnvelope).toLowerCase();
    if (!stopAssist.ok || !stopText.includes("does not automatically block")) {
      throw new Error("Assistant must confirm creation is not blocked/merged/suppressed.");
    }
    ...
    if (serveMode) {
      console.log(
        [
          "",
          "Pack 03 isolated API serving for browser matrix.",
          `  API: ${baseUrl}`,
          ...
          "  Strong-overlap draft title: copy seeded source title/description",
